    ...
    public function __invoke(
        ...
        if (isset($_SESSION['SESSION'])) {
            for ($i=0; $i < count($_SESSION['SESSION']); $i++) {
                if (time() - $_SESSION['CREATE_'.$_SESSION['SESSION'][$i]['setting']['name']] > $_SESSION['SESSION'][$i]['setting']['lifetime'] && !$_SESSION['SESSION'][$i]['setting']['autorefresh']) {
                    // *: Borramos las sesiones ...
                    unset($_SESSION['CREATE_'.$_SESSION['SESSION'][$i]['setting']['name']]);
                    unset($_SESSION[$_SESSION['SESSION'][$i]['setting']['name']]);
                    unset($_SESSION['SESSION'][$i]);
                    sort($_SESSION['SESSION']);
                } else {
                    // *: Actualizamos el tiempo ...
                    $_SESSION['CREATE_'.$_SESSION['SESSION'][$i]['setting']['name']] = time();
                }
            }
        }
